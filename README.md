...
| `GET /v1/history/{job}` | **durable anomaly history** over `?from=&to=` (needs `serve --history DIR`) |
| `GET /v1/catalog` · `/v1/catalog/{name}` | ready-made **job templates** (nginx/kubernetes/postgres/redis/jvm) |
| `GET /v1/jobs/{name}/stale` · `POST …/feedback` | **staleness** watchdog (silent series) · **false-positive feedback** |
| `GET /openapi.json` | the full REST surface as an **OpenAPI 3** document |
...
| **Hardening ✅ (v0.5)** | Precision & correctness pass: concurrency **stress test** + `make race`; **NaN/Inf** input hardening across all model families; **parser fuzzing** (found & fixed a snappy **decompression-bomb DoS**); **determinism** (byte-identical output; fixed influencer tie-breaks) + **snapshot fixed-point** (fixed unpersisted drift state); **score calibration** (~0% FP on noise) + **golden** F1=1.0 test; **NAB** corpus scoring (`semeion nab`); **benchmarks** + memory-bound test. See CHANGELOG. |
| **Parity ✅** | Elastic-ML parity + precision pass from an honest gap audit (each tested): **per-series score renormalization** (an extreme series no longer crushes others), **model-plot bounds** for distribution/population/geo/multivariate, **rarity-scaled** rare score, combined **`over` + `by`/`partition`** population, **delayed-data grace** (`query_delay`), **`summary_count_field`**, **`skip_model_update`** rule action, **per-partition count zero-fill**, **recurring calendars** (daily/weekly), and **`multi_bucket_impact`** in records. See CHANGELOG. |
| **Depth ✅** | Eight more capabilities (each tested): **ratio/SLI** detector, **Holt-Winters (ETS)** forecasting, **ensemble/voting** score combiner, **datafeed staleness** watchdog (`/v1/jobs/{name}/stale`), **durable history** store + query (`/v1/history`, `serve --history`), **feedback-driven suppression** (`/v1/jobs/{name}/feedback`), ready-made **job catalog** (nginx/k8s/postgres/redis/jvm, `/v1/catalog`), and an **OpenAPI 3** spec (`/openapi.json`). See CHANGELOG. |
